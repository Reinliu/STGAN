...
def load_data(path, dims):
    data = []
    for filename in glob.glob(os.path.join(path, '*.png')):
        specs = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)
        specs = np.array(specs)
        specs = specs.reshape([-1, dims]).squeeze()
        data.append(specs)
...
def main():
    number_of_bins = 100
    categories = ['Dogbark', 'Footstep', 'Gunshot', 'Keyboard', 'Motor', 'Rain', 'Sneeze_Cough']
    train_path = '/home/rein/Downloads/dev-data-melspecs/'  
    test_path = '/home/rein/Documents/GAN-conv1d/images/'  
    dims = 64 * 400  

    with open('ndb_results.txt', 'w') as file:
        for cat in categories:
            train_samples = load_data(os.path.join(train_path, cat), dims)
            test_samples = load_data(os.path.join(test_path, cat), dims)
            print(train_samples.shape, test_samples.shape)

            ndb = NDB(training_data=train_samples, number_of_bins=number_of_bins, whitening=True)
            results = ndb.evaluate(test_samples, model_label=cat)

            file.write(f'Category: {cat}, NDB Score: {results["NDB"]}, JS Divergence: {results["JS"]}\n')
